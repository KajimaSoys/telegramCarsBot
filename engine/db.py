...
def select_find(table: str, search_str: str, columns: List[str]) -> List[Tuple]:
...
def select_compare(table: str, search_str: str, columns: List[str]) -> List[Tuple]:
    columns_joined = ", ".join(columns)
    cursor.execute('SELECT set_limit(0.25);')
    conn.commit()
    cursor.execute(f"""
        SELECT {columns_joined}, similarity(search_string, '{search_str}') as sml
        FROM {table} WHERE search_string % '{search_str}'
